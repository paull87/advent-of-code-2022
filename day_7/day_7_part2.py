...
def large_dirs(dir, max_size):
    ...
    if dir.size >= max_size:
        output_dirs.append(dir)
    for sub_dir in dir.contains.values():
        output_dirs += large_dirs(sub_dir, max_size)
    return output_dirs


def calculate_required_deletes(dir, total, required):
    disk_needed = required - (total - dir.size)
    candidates_for_del = large_dirs(dir, disk_needed)
    return sorted(candidates_for_del, key=lambda x: x.size)[0]
...
def main():
    ...
    to_del = calculate_required_deletes(dir, DISK_LIMIT, UPDATE_SPACE)

    print(f"Dir to delete: {to_del.name}, size: {to_del.size}")
